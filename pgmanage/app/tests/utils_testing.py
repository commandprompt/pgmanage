...
def get_client_omnidb_session(p_client=None):
    ...
    if 'pgmanage_session' in p_client.session:
        return p_client.session['pgmanage_session']
...
def execute_client_login(p_client=None, p_username=None, p_password=None):
    ...
    # trigger check_session request to populate pgmanage_session var
    p_client.get(
        reverse('check_session'),
        {},
        follow=True
    )
